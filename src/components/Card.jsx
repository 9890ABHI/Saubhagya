        ...
        >
          ...
          >
            <div className="w-[100%] px-2 pt-2 text-black flex flex-col-reverse">
              <Typography variant="subtitle2" className="text-sm hidden md:flex">{item.title}</Typography>
              <Typography variant="subtitle2" className="text-sm flex md:hidden">{item.title.slice(0, 50) + '...'}</Typography>
              <div className="flex flex-col w-[100%] gap-1 justify-start md:justify-center md:flex-col-reverse" >
                <Box className="flex gap-2 ">
               <Typography variant="subtitle2" className="text-red-400" >{offerOfProduct}%</Typography>
                <Typography variant="subtitle2" className="line-through text-gray-400">₹ {item.oldPrice} </Typography>
                </Box>
                <Box className="flex">
                <Typography variant="subtitle2" className="text-sm text-gray-800">MRP: ₹</Typography>
                <Typography variant="subtitle2" className="text-sm text-gray-700">{item.price}</Typography>
                </Box>
...
export const CartCard = ({ item, setFilterProds, filterProds , Feature }) => {
        ...
        <Box className="flex items-center overflow-hidden justify-between ">
          <Box className="md:w-[20%]">
            <img
              src={item?.image}
              alt=""
              className={`w-full h-[100px] object-cover overflow-hidden rounded-md ${Feature && 'w-[85px] h-[95px]'}`}
            />
          </Box>
          <Box className={` w-[70%] md:w-[90%] flex justify-between items-center text-left`}> 
          <Box className={`w-[50%] md:w-[80%]  ${Feature && 'w-[85%] pl-2'}`}>
          ...
          <Typography sx={{ fontFamily: "Regular" }} className="hidden max-sm:flex ">
            ...
            <Box className={`flex ${Feature && 'hidden'} justify-between pt-3`}>
              <Box className="flex gap-2 items-center">
                {/* <Button
                  ...
                  <Remove onClick={() => handleRemove(item?.id)} color="primary"/>
                  </Box>
                {/* </Button> */}
                <Box className="flex px-3 py-2 rounded-md border ">
                ...
                {/* <Button
                  ...
                  <Add onClick={() => setQuantity(quantity + 1)} color="primary" />
                  </Box>
                {/* </Button> */}
              </Box>
              <Button
          ...
          <Box className={` ${Feature ? 'w-[55%] flex flex-col gap-2 justify-between pl-2' :' w-[80%] md:w-[20%] max-sm:pl-6 max-sm:pb-6 pl-6'}`}>
            <Typography
              variant="body1"
            ...
            </Typography>
            {
              Feature && (
                <>
                <Button variant="contained" sx={{color:"#fff" , backgroundColor:'#000',"&.MuiButton-root": {
            // background: "linear-gradient(to left,  #f88b6965, #c90f7270)",
            background: "linear-gradient(to right,  #c90f72, #c90f7290)",
          }, fontSize:10, fontWeight:700 , textAlign:"center" , display:'flex' , alignItems:'center' , gap:1 }}>
...
export const OrderCard  = ({item}) => {
  return(
    ...
    <Box className="flex justify-between md:w-[75%] max-sm:w-[95%] items-center md:px-10 px-3 py-2 bg-[#fff] rounded-md">
      <Box>
    <img className="md:w-[100px] md:h-[100px] h-[] object-cover rounded-md" alt="" src={item.image}/>
      </Box>
      ...
      <Box className="flex flex-col justify-start items-start">
        <Typography>
          Deliverd on Nov 01,2023
        </Typography>
        <Typography>
          your item has been deliver
        </Typography>
